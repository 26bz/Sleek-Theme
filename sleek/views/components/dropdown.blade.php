<div class="relative" x-data="{ open: false, adjustWidth: 0 }" x-init="$watch('open', value => {
    ...
    <button
        class="flex flex-row items-center px-2 py-1 text-sm font-semibold whitespace-nowrap text-base hover:text-primary transition-colors duration-200"
    ...
    <div x-ref="dropdown"
        class="absolute mt-2 w-48 px-2 py-1 bg-background-secondary rounded-lg shadow-xl z-10 border border-neutral/20 overflow-hidden"
        x-bind:style="{
            left: `-${adjustWidth}px`,
        }" x-show="open"
